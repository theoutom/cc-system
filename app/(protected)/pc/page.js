    ...
    <div>
      <div className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Sistem billing pemakaian komputer (Max 4 PC)</p>
        </div>
        <button onClick={() => setShowForm(true)} disabled={activeSessions.length >= 4}
            ...
            <div key={session.id} className={`bg-white rounded-2xl p-5 border shadow-sm flex flex-col h-48 ${
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="font-bold text-slate-800 mt-1 line-clamp-1">{session.nama_pengguna}</p>
                </div>
